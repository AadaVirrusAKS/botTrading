...
def check_web_app_exists():
    """Verify app/web_app.py exists"""
    if not os.path.exists('app/web_app.py'):
        print(f"{RED}❌ Error: app/web_app.py not found in project root{RESET}")
...
def launch_web_app():
    ...
    try:
        # Run the web app via run.py entry point
